...
const sl = (p: Piece, t: number[], s: number[], n: number[]): number[] => select(p.left, t, s, n);
const sr = (p: Piece, t: number[], s: number[], n: number[]): number[] => select(p.right, t, s, n);
const su = (p: Piece, t: number[], s: number[], n: number[]): number[] => select(p.up, t, s, n);
const sd = (p: Piece, t: number[], s: number[], n: number[]): number[] => select(p.down, t, s, n);
...
export class Squared implements Outline {
  draw(piece: Piece, size: Vector | number = 50, borderFill: Vector | number = 0): number[] {
    ...
    const offset = VectorModule.divide(VectorModule.multiply(borderFill, 5), sizeVector);
    const selNum = (insert: Insert, t: number, s: number, n: number) => select(insert, t, s, n);
    return [
      (0 - offset.x),                                                  (0 - offset.y),
      1,                                                               (0 - offset.y),
      2,                                                               selNum(piece.up, (-1 - offset.y), (1 - offset.y), (0 - offset.y)),
      3,                                                               (0 - offset.y),
      (4 + offset.x),                                                  (0 - offset.y),
      (4 + offset.x),                                                  1,
      selNum(piece.right, (5 + offset.x), (3 + offset.x), (4 + offset.x)), 2,
      (4 + offset.x),                                                  3,
      (4 + offset.x),                                                  (4 + offset.y),
      3,                                                               (4 + offset.y),
      2,                                                               selNum(piece.down, (5 + offset.y), (3 + offset.y), (4 + offset.y)),
      1,                                                               (4 + offset.y),
      (0 - offset.x),                                                  (4 + offset.y),
      (0 - offset.x),                                                  3,
      selNum(piece.left, (-1 - offset.x), (1 - offset.x), (0 - offset.x)), 2,
      (0 - offset.x),                                                  1
...
export class Rounded implements Outline {
  ...
  referenceInsertAxisLength(fullSize: Vector): number {
    return this.referenceInsertAxis ? this.referenceInsertAxis.atVector(fullSize) : VectorModule.inner.min(fullSize);
  }

  draw(p: Piece, size: Vector | number = 150, _borderFill: Vector | number = 0): number[] {
