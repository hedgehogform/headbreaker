...
describe('canvas', () => {
  ...
  it('passes shape variation options to autogenerated puzzles', () => {
    const canvas = new Canvas('canvas', {
      width: 800,
      height: 800,
      painter,
    ...
    canvas.autogenerate({
      ...
      shapeVariation: { offset: 0, width: 0, depth: 0 },
    });

    expect(canvas.puzzle.pieces[0].shape.right).toEqual({
      offset: 0,
      width: 1,
      depth: 1,
    });
  ...
  it('can autogenerate with legacy uniform outlines', () => {
    const canvas = new Canvas('canvas', {
      width: 800,
      height: 800,
      painter,
    ...
    expect(canvas.puzzle.pieces[0].shape).toEqual({});
    expect(canvas.puzzle.pieces[1].shape).toEqual({});
  });
